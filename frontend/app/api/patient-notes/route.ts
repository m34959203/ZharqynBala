...
// GET /api/patient-notes - Get all patient notes for psychologist
export async function GET(request: Request) {
  try {
    ...
    let url = `${API_URL}/api/v1/patient-notes`;
    const params = new URLSearchParams();
    if (clientId) params.append('clientId', clientId);
    if (consultationId) {
      url = `${API_URL}/api/v1/patient-notes/consultation/${consultationId}`;
...
// POST /api/patient-notes - Create a patient note
export async function POST(request: Request) {
  try {
    ...
    const { response, newAccessToken } = await makeAuthenticatedRequest(
      `${API_URL}/api/v1/patient-notes`,
      { method: 'POST', body: JSON.stringify(body) },
      tokens
    );

    if (!response.ok) {
      const error = await response.json().catch(() => ({ message: 'Ошибка создания заметки' }));
      return NextResponse.json({ error: error.message }, { status: response.status });
    }
    ...
    return NextResponse.json({ error: 'Failed to create patient note' }, { status: 500 });
  }
}
