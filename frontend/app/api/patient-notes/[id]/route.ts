...
// GET /api/patient-notes/[id] - Get a specific patient note
export async function GET(
  ...
  try {
    ...
    const { response, newAccessToken } = await makeAuthenticatedRequest(
      `${API_URL}/api/v1/patient-notes/${id}`,
      { method: 'GET' },
      tokens
    );

    if (!response.ok) {
      const error = await response.json().catch(() => ({ message: 'Заметка не найдена' }));
    ...
    return NextResponse.json({ error: 'Failed to fetch patient note' }, { status: 500 });
  }
}

// PUT /api/patient-notes/[id] - Update a patient note
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to update patient note' }, { status: 500 });
  }
}

// DELETE /api/patient-notes/[id] - Delete a patient note
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete patient note' }, { status: 500 });
  }
}
