...
// GET /api/consultations - Получить мои консультации (клиент)
// GET /api/consultations?psychologist=true - Получить консультации психолога
export async function GET(request: Request) {
  try {
    ...
    const isPsychologist = searchParams.get('psychologist') === 'true';
...
// POST /api/consultations - Записаться на консультацию
export async function POST(request: Request) {
  try {
    ...
    const { response, newAccessToken } = await makeAuthenticatedRequest(
      `${API_URL}/api/v1/consultations`,
      { method: 'POST', body: JSON.stringify(body) },
      tokens
    );

    if (!response.ok) {
      const error = await response.json().catch(() => ({ message: 'Ошибка создания консультации' }));
      return NextResponse.json({ error: error.message }, { status: response.status });
    }
