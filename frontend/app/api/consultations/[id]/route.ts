...
export async function GET(
  ...
  try {
    ...
    const { response, newAccessToken } = await makeAuthenticatedRequest(
      `${API_URL}/api/v1/consultations/${id}`,
      { method: 'GET' },
      tokens
    );
...
// PUT /api/consultations/[id] - Обновить консультацию (действия)
// Body: { action: 'confirm' | 'reject' | 'cancel' | 'start' | 'complete' | 'no-show' | 'rate', ...data }
export async function PUT(
  ...
  try {
    ...
    const validActions = ['confirm', 'reject', 'cancel', 'start', 'complete', 'no-show', 'rate'];
    if (!validActions.includes(action)) {
      return NextResponse.json({ error: 'Invalid action' }, { status: 400 });
    }

    const { response, newAccessToken } = await makeAuthenticatedRequest(
      `${API_URL}/api/v1/consultations/${id}/${action}`,
      { method: 'PUT', body: JSON.stringify(data) },
      tokens
    );

    if (!response.ok) {
      const error = await response.json().catch(() => ({ message: 'Ошибка операции' }));
      return NextResponse.json({ error: error.message }, { status: response.status });
    }
