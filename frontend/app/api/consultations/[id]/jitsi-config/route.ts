...
// GET /api/consultations/[id]/jitsi-config - Получить конфигурацию Jitsi для видеозвонка
export async function GET(
  ...
  try {
    ...
    const { response, newAccessToken } = await makeAuthenticatedRequest(
      `${API_URL}/api/v1/consultations/${id}/jitsi-config`,
      { method: 'GET' },
      tokens
    );

    if (!response.ok) {
      const error = await response.json().catch(() => ({ message: 'Ошибка загрузки конфигурации' }));
    ...
    return NextResponse.json({ error: 'Failed to fetch Jitsi config' }, { status: 500 });
  }
}
