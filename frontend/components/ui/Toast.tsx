...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const addToast = useCallback((toast: Omit<Toast, 'id'>) => {
    // Use crypto.randomUUID() for secure, unique IDs instead of Math.random()
...
function ToastContainer({ toasts, onRemove }: { toasts: Toast[]; onRemove: (id: string) => void }) {
  return (
    <div
      className="fixed bottom-4 right-4 z-50 flex flex-col space-y-2 max-w-md"
      role="region"
...
function ToastItem({ toast, onRemove }: { toast: Toast; onRemove: (id: string) => void }) {
  ...
  return (
    ...
    >
      <div className={cn('flex-shrink-0', iconStyles[toast.type])} aria-hidden="true">
        {icons[toast.type]}
      </div>
